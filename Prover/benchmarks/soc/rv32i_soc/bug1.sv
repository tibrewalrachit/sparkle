...
module rv32i_soc (
    ...
    // BUG (seeded): decoder regression — SC.W is classified as a
    // memory-to-register instruction (id_isAMO instead of id_isAMOrw),
    // so a store-conditional is routed through the load writeback path.
    wire id_memToReg = id_isLoad | id_isLR | id_isAMO;

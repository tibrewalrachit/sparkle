...
instruction-address-misaligned trap). On a 12-design HWMCC-style benchmark
suite with 19 seeded bugs, \sfv{} resolves more instances than the
\tool{yosys-smtbmc} baseline (the engine behind SymbiYosys) under identical
elaborations, and confirms every found bug in simulation.
\end{abstract}
...
\paragraph{Discussion.} Three observations. (1)~On bug variants both tools
are complete up to the bound, so the differentiator is proof power on safe
variants: the baseline's temporal induction proves only $k$-inductive
properties, while \sfv{} additionally closes non-inductive properties via
Houdini strengthening --- the classic FIFO/counter-bound cases. (2)~Verilator
confirmation adds negligible cost relative to solving and converts every
formal finding into a simulation artifact. (3)~At SoC scale, word-level
arrays keep the model compact; the SoC's eight architectural invariants
prove in seconds per property, indicating that the practical bottleneck for
full-SoC convergence is property authoring --- which is precisely the step
